...
const PositionCard: React.FC<Props> = ({ position }) => {
    ...
    const investment = position.quantity * position.price;
    ...
    return (
        <TableRow className="border-b dark:border-gray-800 hover:bg-gray-100 dark:hover:bg-gray-800 transition-all duration-200">
            ...
            <TableCell className="px-6 py-3 md:text-xl  text-gray-700 dark:text-gray-300">{position.quantity.toLocaleString()}</TableCell>
            <TableCell className={`px-6 py-3 md:text-xl  font-medium ${position.price < position.ltp ? 'text-green-600' : 'text-red-600'}`}>
                Rs {position.price.toLocaleString()}
            </TableCell>
